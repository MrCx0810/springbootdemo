...
function editHall(id) {
    layer.open({
        ...
        maxmin: true,
        area: ['1400px', '700px'],
        content: "../layer/editHall?id="+id,
    });
}

function delHall(id) {
    layer.confirm("你确认删除此首页文章？", {icon: 3, title:'提示'}, function(index){
        $.post("../admin/deleteHall",{id:id},function(e){
            if(e.code==100){
...
$('#up_art').on('click', function () {
    var data=$("#create-art").serialize();
    console.log(data);
    $.post("../admin/updateHall",data,function(e){
        if(e.code==200){
